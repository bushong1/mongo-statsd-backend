...
var connection_queue = async.queue(function(task, callback) {
	if(dbs[task.name]) {
		...
		new mongo.Db(task.name, new mongo.Server(options.host, options.port), {w:'majority'}).open(function(err, db) {
			if (err) {
				callback(err);
			} else {
				dbs[task.name] = db;
...
var insert = function(dbName, collection, metric, callback) {
	...
	database(dbName, function(err, db) {
      ...
      if (err) {
        ...
        db.ensureIndex("time", function(err, indexName) {
          if (err) {
            callback(err);
          } else {
